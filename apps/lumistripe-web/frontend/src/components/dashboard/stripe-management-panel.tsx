...
export function StripeManagementPanel({ controller }: { controller: DashboardController }) {
  ...
  const [draft, setDraft] = useState<StripeTopology | null>(null)
  const [dirty, setDirty] = useState(false)

  useEffect(() => {
    if (state && !dirty) setDraft(copyTopology(state.stripe_topology))
  }, [dirty, state])
  ...
  const updateOutput = (index: number, patch: Partial<StripeOutputConfig>) => {
    setDraft((current) => current && ({ ...current, outputs: current.outputs.map((output, item) => item === index ? { ...output, ...patch } : output) }))
    setDirty(true)
  }

  const apply = async () => {
    if (draft.outputs.some((output) => !output.name.trim())) return toast.error("Every stripe needs a name.")
    const ok = await controller.updateStripes(draft)
    if (ok) {
      setDirty(false)
  ...
  const remove = (index: number) => {
    ...
    if (!window.confirm(`Remove ${output.name}? The output will be turned off when you apply.`)) return
    setDraft({ ...draft, outputs: draft.outputs.filter((_, item) => item !== index) })
    setDirty(true)
  }
  ...
  return (
    <SetupPage>
      <Card className="border-white/5 bg-card/80">
        ...
        <CardContent>
          <ToggleGroup aria-label="Stripe layout" value={[draft.layout]} onValueChange={(values) => { const value = values[0] as StripeLayout | undefined; if (value) { setDraft({ ...draft, layout: value }); setDirty(true) } }} className="grid w-full grid-cols-3 gap-1 rounded-xl bg-black/20 p-1" variant="outline">
      ...
      {draft.outputs.map((output, index) => (
        <Card key={output.id} className="border-white/5 bg-card/80">
          <CardHeader>
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="flex gap-1">
                ...
                <Button variant="ghost" size="icon" disabled={busy} onClick={() => remove(index)} aria-label="Remove stripe"><Trash2Icon className="text-red-300" /></Button>
              </div>
            </div>
          </CardHeader>
          <CardContent className="space-y-4">
            <Field label="Name"><Input value={output.name} maxLength={40} onChange={(event) => updateOutput(index, { name: event.target.value })} /></Field>
            <Field label="Pixel count"><Input type="number" min={1} max={4096} inputMode="numeric" value={output.pixels} onChange={(event) => updateOutput(index, { pixels: Number(event.target.value) })} /></Field>
            ...
            {dirty && <p className="text-xs text-amber-300">Tests use this draft temporarily, then restore the applied setup.</p>}
          </CardContent>
        </Card>
      ))}

      {draft.outputs.length < 2 && <Button variant="outline" className="h-12 w-full rounded-xl" disabled={busy} onClick={() => { setDraft({ ...draft, outputs: [...draft.outputs, newStripe(draft.outputs)] }); setDirty(true) }}><PlusIcon />Add stripe</Button>}
      <Button className="h-14 w-full rounded-2xl" disabled={!dirty || busy} onClick={() => void apply()}><SaveIcon />{busy ? "Applying…" : "Save & apply"}</Button>
